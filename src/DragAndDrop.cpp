...
bool OnDrop(int x, int y) {
  ...
  if (DragAndDrop::HoveredControl) {
    ...
    bSuccess =
        DragAndDrop::HoveredControl->DragAndDrop_HandleDrop(DragAndDrop::CurrentPackage, x, y);
  }
...
bool ShouldStartDraggingControl(int x, int y) {
  ...
  DragAndDrop::CurrentPackage =
      LastPressedControl->DragAndDrop_GetPackage(LastPressedPos.x, LastPressedPos.y);
...
void DragAndDrop::RenderOverlay(Gwen::Controls::Canvas * /*pCanvas*/, Skin::Base *skin) {
  ...
  skin->GetRender()->AddRenderOffset(
      Gwen::Rect(m_iMouseX - SourceControl->X() - CurrentPackage->holdoffset.x,
