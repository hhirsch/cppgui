...
bool Gwen::Input::OnMouseClicked(Controls::Base *pCanvas, int iMouseButton, bool bDown) {
  ...
  if (bDown && g_pntLastClickPos.x == MousePosition.x && g_pntLastClickPos.y == MousePosition.y &&
  ...
  if (bDown) {
    if (Hook::CallHook(&Hook::BaseHook::OnControlClicked, Gwen::HoveredControl, MousePosition.x,
                       MousePosition.y)) {
      return true;
    }
  }
