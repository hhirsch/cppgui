...
void Text::Render(Skin::Base *skin) {
  ...
  skin->GetRender()->RenderText(GetFont(), Gwen::Point(GetPadding().left, GetPadding().top),
                                m_String.GetUnicode());
}
...
Gwen::Rect Text::GetCharacterPosition(int iChar) {
  ...
  if (Length() == 0 || iChar == 0) {
    Gwen::Point p = GetSkin()->GetRender()->MeasureText(GetFont(), U" ");
...
void Text::RefreshSizeWrap() {
  ...
  Point pFontSize = GetSkin()->GetRender()->MeasureText(GetFont(), U" ");
  int w = GetParent()->Width() - GetParent()->GetPadding().left - GetParent()->GetPadding().right;
  int x = 0, y = 0;
