...
Base::~Base() {
  ...
  for (AccelMap::iterator accelIt = m_Accelerators.begin(); accelIt != m_Accelerators.end();
       ++accelIt) {
...
void Base::BringNextToControl(Controls::Base *pChild, bool bBehind) {
  ...
  Base::List::iterator it =
      std::find(m_ActualParent->Children.begin(), m_ActualParent->Children.end(), pChild);
...
void Base::RecurseLayout(Skin::Base *skin) {
  ...
  for (Base::List::iterator iter = Children.begin(); iter != Children.end(); ++iter) {
    ...
    if (iDock & Pos::Left) {
      const Margin &margin = pChild->GetMargin();
      pChild->SetBounds(rBounds.x + margin.left, rBounds.y + margin.top, pChild->Width(),
