...
public:
  ...
  template <typename T>
  void AddAccelerator(const TextObject &accelerator, void (T::*func)(Gwen::Event::Info),
                      Gwen::Event::Handler *handler = NULL, void *data = NULL) {
    if (handler == NULL) {
      handler = this;
    }
  ...
  template <typename T>
  void GlobalAddAccelerator(const TextObject &accelerator, T func,
  ...
  void GlobalAddAccelerator(const TextObject &accelerator, void (*func)(Gwen::Event::Info),
                            Gwen::Event::Handler *handler = NULL, void *data = NULL) {
    if (handler == NULL) {
      handler = this;
    }
...
public:
  ...
  virtual void SetAction(Event::Handler *pObject, Handler::FunctionWithInformation pFunction,
...
template <class T> T *gwen_cast(Gwen::Controls::Base *p) {
  if (!p) {
    return NULL;
  }
...
public:                                                                                            \
  ...
  virtual const char *GetBaseTypeName() {                                                          \
